...
export class ConfigService {
  ...
  private static async populateEnvFromGCPSecretManager(envConfig: EnvConfig, initialConfigs: EnvConfig): Promise<EnvConfig> {
    ...
    const envVarMapping = {
      ...
      DB_CONNECTION_LIMIT: 'M_API_DB_CONNECTION_LIMIT',
    ...
    for (const key in envVarMapping) {
      try {
        ...
        const [version]: GCPSM = await ConfigService.secretManagerServiceClient.accessSecretVersion({
