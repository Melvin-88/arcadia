import { Logger as BuildInLogger, Inject } from '@nestjs/common';
import {
  Configuration,
  Logger as Logger4js,
  configure,
...
export class AppLogger extends BuildInLogger {
  ...
  accessRpc(request: TcpContext | RedisContext, executionTime = 0) {
    ...
    const args: TcpContext = <TcpContext><unknown>request.getArgs();

    const message = {
      pattern: request.getPattern(),
      body: args[0].content.toString(),
      executionTime: executionTime > 0 ? Date.now() - executionTime : 0,
    };
  ...
  error(message: string, trace?: string | Record<string, any>) {
