...
export class SessionLocale1591879561908 implements MigrationInterface {
    name = 'SessionLocale1591879561908'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("ALTER TABLE `session` ADD `locale` varchar(10) NOT NULL DEFAULT 'en_US'", undefined);
        await queryRunner.query("ALTER TABLE `session_archive` ADD `locale` varchar(10) NOT NULL DEFAULT 'en_US'", undefined);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("ALTER TABLE `session_archive` ADD CONSTRAINT `FK_9e6c05c3a983b1f44512f045bac` FOREIGN KEY (`queue_id`) REFERENCES `queue`(`id`) ON DELETE RESTRICT ON UPDATE NO ACTION", undefined);
        await queryRunner.query("ALTER TABLE `session` ADD CONSTRAINT `FK_be1b2b01519269226ae6a7f4cd3` FOREIGN KEY (`queue_id`) REFERENCES `queue`(`id`) ON DELETE RESTRICT ON UPDATE NO ACTION", undefined);
        await queryRunner.query("ALTER TABLE `session_archive` DROP COLUMN `locale`", undefined);
        await queryRunner.query("ALTER TABLE `session` DROP COLUMN `locale`", undefined);
    }
